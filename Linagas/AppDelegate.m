...
#if DISTRIBUTION == 1
    [GMSServices provideAPIKey:kGoogleMapsAPIKey_Distributor];
#else
    [GMSServices provideAPIKey:kGoogleMapsAPIKey_Consumer];
#endif
    
    if (![Common objectInDataCacheForKey:kFirstTimeUse])
    {
        [Common setBool:YES inDataCacheForKey:kFirstTimeUse];
        [self clearKeychain];
    }
...
- (void)clearKeychain
{
    ...
    [SSKeychain deletePasswordForService:kUserPassword account:kApplicationID];
}
